...
def New_REG():
    subprocess.run(["python", "MSCIT.py"])  # Run the Tally.py script in a new process

def Attend():
    subprocess.run(["python", "MSCIT_Attend.py"])  # Run the Tally_Attend.py script

def TallyDB():
    subprocess.run(["python", "MSCIT_DB.py"])
...
bg_image = tk.PhotoImage(file="Images/MSCIT.png") 
