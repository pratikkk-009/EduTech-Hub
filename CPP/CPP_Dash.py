...
def New_REG():
    subprocess.run(["python", "CPP.py"])  # Run the Tally.py script in a new process

def Attend():
    subprocess.run(["python", "CPP_Attend.py"])  # Run the Tally_Attend.py script

def TallyDB():
    subprocess.run(["python", "CPP_DB.py"])
...
bg_image = tk.PhotoImage(file="Images/CPP.png") 
