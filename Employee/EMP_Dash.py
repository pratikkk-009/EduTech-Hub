...
def New_REG():
    subprocess.run(["python", "EMP.py"])  # Run the Tally.py script in a new process

def Attend():
    subprocess.run(["python", "EMP_Attend.py"])  # Run the Tally_Attend.py script

def TallyDB():
    subprocess.run(["python", "EmpDB.py"])
...
bg_image = tk.PhotoImage(file="Images/emp.png") 
bg_label = tk.Label(root, image=bg_image)
bg_label.place(relwidth=1, relheight=1)

# Load button images
button1_image = tk.PhotoImage(file="Images/Reg_Emp.png")
button2_image = tk.PhotoImage(file="Images/Attend_Button.png")  
button3_image = tk.PhotoImage(file="Images/Details.png") 
