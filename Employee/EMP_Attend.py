...
try:
    ...
    if "Employee ID" not in students_df.columns or "Name" not in students_df.columns:
        raise KeyError("The 'Sheet' must have columns: 'Employee ID' and 'Name'.")
    print("Students DataFrame Loaded:")
    
except Exception as e:
    messagebox.showerror("Error", f"Failed to load students data: {e}")
    students_df = pd.DataFrame(columns=["Employee ID", "Name"])
...
try:
    ...
    attendance_df.set_index("Employee ID", inplace=True)
    print("Attendance DataFrame Loaded:")
   
except Exception as e:
    print(f"Attendance data not found or invalid: {e}")
    attendance_df = students_df.set_index("Employee ID")[["Name"]]
...
for i in range(0, rows, columns):
    ...
    for j in range(columns):
        ...
        if index < rows:
            ...
            attendance_vars[student['Employee ID']] = var
