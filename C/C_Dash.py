...
def New_REG():
    subprocess.run(["python", "C.py"])  # Run the Tally.py script in a new process

def Attend():
    subprocess.run(["python", "C_Attend.py"])  # Run the Tally_Attend.py script

def TallyDB():
    subprocess.run(["python", "C_DB.py"])
...
bg_image = tk.PhotoImage(file="Images/C.png") 
