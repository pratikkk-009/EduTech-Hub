...
def New_REG():
    subprocess.run(["python", "CAD.py"])  # Run the Tally.py script in a new process

def Attend():
    subprocess.run(["python", "CAD_Attend.py"])  # Run the Tally_Attend.py script

def CADDB():
    subprocess.run(["python", "CAD_DB.py"])
...
bg_image = tk.PhotoImage(file="Images/CAD.png") 
...
button3 = tk.Button(root,image=button3_image,width=178,height=178,bd=3,command= CADDB)
